...
// ANSWER: O(1), because it will always iterate through the first 4 elements of the array. No more. It could be less if numbers.length <= 3, but that still gives the same time complexity.


// Q: Could you come up with a solution that has a better time complexity? If so, what is that time complexity?
// ANSWER: answer = numbers[4], the complexity is still O(1)
...
// ANSWER: O(1), I suppose you could do console.log(`${array[0]}\n${array[1]}`), but that wouldn't change the time complexity at all.
...
function squareEveryDigit(number) {
  ...
  let squareNums = []
  for (num of nums) {
    squareNums.push((parseInt(num) ** 2).toString())
  }
  return parseInt(squareNums.join(''))
}

console.log(squareEveryDigit(946)) // When running 946, you should see a result of 811636

// The time complexity of my solution is O(n) becuase it must get each digit in the number and square it.
// I really don't think it can get any more simplified than that because you have to look at each digit and square it.
